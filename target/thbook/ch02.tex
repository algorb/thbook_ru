...
* |projection <specification>| = указывает проекцию чертежа. Каждая проекция идентифицируется типом и дополнительно индексом в форме |тип[:индекс]|. Индекс может быть любым ключевым словом. Поддерживаются следую\-щие типы проекций: 
...
* |scale <specification>| = используется для предварительного масштабирования (преобразования координат пикселей в метры) данных скрапа. Если проекция скрапа установлена в none, то это единственное преобразование, которое выполняется с координатами. |<specification>| имеет четыре формы: 

1. |<number>| = |<number>| метров на единицу чертежа. 

2. |[<number> <length units>]| = |<number> <length units>| на единицу чертежа. 
