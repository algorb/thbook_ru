...
* |format/fmt <формат>| = установить формат списка возможных продолже\-ний (перспектив). В настоящее время поддерживаются следующие форматы вывода: |html| (по умолчанию), |txt|, \NEW{5.4}|kml|\[Для списка пещер и списка продолжений (перспектив).] и |dbf|. 
