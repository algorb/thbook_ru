...
* \NEW{5.4.1}|min-symbol-scale <масштаб>| = определить минимальный |<масштаб>|, при кото\-ром точечные и линейные символы отображаются на карте. E.g. for |min-symbol-scale M|, no points or lines scaled |S| and |XS| will be shown on the map. |<scale>| has the same format, as |scale| option for points and lines. 
