...
|normal/reverse| = развернуть данную и следующие станции в таком же/обратном направлении по отношению к предыдущей станции. Если заданы две станции -- направление применяется только к данному замеру. 

|left/right| =как указано выше, но направление указано явно. 

|vertical| = не перемещайте станцию (замер) в направлении $X$, использует только $Z$ компонент замера. 

|start| = указать начальную станцию (замер) 

|ignore| = игнорировать указанную станцию (замер), продолжить развертку как у другой станции (замера), если возможно 

|hide| = не показывать указанную станцию (замер) в развертке 

Если ни одна из станций не указана, |<spec>| действует для следующих указанных замеров. 

* |station-names <префикс> <суффикс>| = добавляет данный префикс/суффикс для всех станций съемки в текущей нитки хода. Saves some typing. 
