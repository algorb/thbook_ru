...
%-2.35cm
\chapter »зменение макета PDF-карт. 

Ёта глава чрезвычайно полезна, если вас не устраивает предопределенный макет символов карты и ее предоставление, и вы хотите адаптировать их к вашим желани€м. “ем не менее, вам нужно знать, как писать простые макросы \TeX\ и \MP, чтобы добитьс€ этого. 

\subchapter ћакет страницы в режиме атласа. 

 оманда |layout| позвол€ет произвести базовую настройку страницы в режиме атласа. Ёто делаетс€ с помощью таких опций, как |page-setup| или |overlap|. Ќо нет никаких опций, которые указывали бы положение карты, навигатора и других элементов внутри области, определ€емой |page-width| и |page-height|; например, почему навигатор находитс€ ниже карты, а не справа или слева? 

—уществует много возможных вариантов дл€ страницы. ¬место того, чтобы использовать еще больше опций дл€ команды |layout|, Therion использует €зык \TeX\  дл€ описани€ других макетов страниц. 

%(See the section `layout' where to put \TeX\ commands.) 
ѕреимущество такого подхода состоит в том, что пользователь имеет пр€мой доступ к расширенному механизму набора текста, не дела€ €зык Therion сверхсложным. 

Therion использует pdf\TeX\  с форматом {\it plain} дл€ типографского набора.\break  ѕоэтому вы должны быть знакомы с plain \TeX, если хотите определить новые макеты. 

ѕодробнее о plain \TeX\  смотрите в \list

Knuth, D. E.: {\it The \TeX book}, Reading, Massachusetts, Addison-Wesley $^1$1984 \endlist

%[...]
ƒл€ освоени€ Pdf\TeX имеетс€ краткое руководство \list

Th\`anh, H. T.---Rahtz, S.---Hagen, H.: {\it The pdf\TeX\ user manual}, available at \hfil\break \www{http://www.pdftex.org} \endlist

\TeX\  макросы используютс€ внутри |code tex-atlas| команды |layout| (подробнее смотрите главу {\it ќбработка данных}). ќсновным определением Therion'а €вл€етс€ макрос 

|\dopage| 

 »де€ проста: дл€ каждой страницы Therion определ€ет переменные \TeX\  (count, token и box registers), которые содержат элементы страницы (карта, навигатор, название страницы и т.д.). At the end of each page macro |\dopage| is invoked. This defines the position of each element on the page. By redefining this macro you'll get desired page layout. Without this redefinition you'll get the standard layout. 
...
* |\pagelabel| = the page label as specified by |origin| and |origin-label| options of the |layout| command. \endlist
...
* |\ifpagenumbering| = This conditional is set true or false according to the |page-numbers| option of the |layout| command. \endlist
...
{\tt|\def\processpointeritem#1|\char124|#2|\char124}|#3\endarg{ \hbox{\pdfstartlink attr {/Border [0 0 0]} goto name {#3} #2 (#1)\pdfendlink} }| 

%% %
(note how to use the {\it destination} argument), or much simpler (if we don't need hyperlink features): 

{\tt|\def\processpointeritem#1|\char124|#2|\char124}|#3\endarg{ \hbox{#2 (#1)} }| \endlist
...
* |\rm|, |\it|, |\bf|, |?|, |\si| for type face switching. \endlist
...
|layout my_layout scale 1 200 page-setup 29.7 21 27.7 19 1 1 cm size 26.7 18 cm overlap 0.5 cm code tex-atlas \def\dopage{\box\mapbox} \insertmaps endlayout| 
