...
Язык вывода по умолчанию. Список доступных языков смотрите на стра\-нице авторских прав; 
...
Работает только в Windows. Когда установлено значение |off| (по умол\-чанию), Therion временно очищает все переменные среды, связанные с \TeX. Полезно, если в вашей системе установлен другой дистрибутив \TeX, который настроил переменные среды, которые могут запутать программы \TeX\ и \MP, поставляемые с дистрибутивом Therion для Windows. 

Установите в |on|, если вы используете другой дистрибутив \TeX\ для обра\-ботки карт; 

* |text <ID языка> <текст Therion'а> <свой текст>| 

С помощью этой опции вы можете изменить любой текст перевода по умолчанию на выходе. Список текстов и доступных переводов смотрите в файле |thlang/texts.txt|; 
...
Для использования этой команды Therion требует установки LCDF Typetools в вашей системе. Пример: 
...
Когда установлено значение |on| (по умолчанию), шрифты OpenType,\break используемые в |pdf-fonts|, преобразуются в шрифты PFB, если они осно\-ваны на PostScript. Некоторая информация теряется в формате PFB, но есть преимущество в том, что pdf\TeX\ может встраивать часть шрифтов PFB (в отличие от шрифтов OpenType, которые должны быть встроены полностью); 
...
За кодировкой должны следовать пять спецификаций шрифтов для обыч\-ных, курсивных, полужирных, без засечек и наклонных без засечек. Зна\-чение по умолчанию: |tex-fonts raw cmr10 cmti10 cmbx10 cmss10 cmssi10|. 
...
в pdf\TeX\ файл карты. То же самое должно быть сделано для курсивых и полужирных шрифтов и соответствующих шрифтов без засечек и наклон\-ных без засечек. Если вы ленитесь, попробуйте: 
...
Если вы хотите добавить некоторые неподдерживаемые кодировки, прочи\-тайте главу {\it Компиляция / Руководство хакера}; 
...
SVG-карта: без ограничений; * количество скрапов = примерно 500--6000, в зависимости от частоты попе\-речных сечений 

текущий предел \MP: $4 (скрапы + сечения) < 4096$ (может быть про\-извольно увеличено) 
...
Декабрь, 27: Therion впервые компилирует простую карту в формате PostScript (32 Кбайт Perl и 7 Кбайт \MP\ и \TeX\ исходного кода). Модель деформации карты существенно отличалась от текущей (пози\-ции характеристик были относительно конкретного съемного замера, а не позиции всех станций в скрапе). Эта версия уже включала некото\-рые интересные функции, такие как {\it функции трансформации}, которые позволяли определять спецификацию входного формата для данных\break съемки или разбивать большие карты на несколько листов. 
...
Март: Therion 0.1 -- Therion способен обрабатывать данные съемки (цен\-тральной линии) пещеры Мертвых летучих мышей. XTherion -- тексто\-вый редактор, предназначенный для Therion'а (Tcl/Tk). 

Июль, 27: Therion 0.2 -- Therion впервые в первый раз компилирует прос\-тую карту (состоящую из двух скрапов) (800 КБ исходного кода). 

Август: XTherion расширен до редактора 2D-карт (в качестве замены ThEdit). 
