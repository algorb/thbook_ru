...
У каждого скрапа есть своя собственная локальная декартова система коор\-динат, которая обычно соответствует миллиметровой бумаге (если Вы определяете координаты символов карты вручную) или пикселям отсканированного изображения (если Вы используете XTherion). Therion делает преобразование этой локальной системы координат в реальные координаты, используя позиции станций съемки, которые определены в скрапе как символы точки на карте и определены в нитке хода. Если скрап не содержит по крайней мере две станции съемки с определением |-name|, тогда Вы должны использовать опцию |-scale| для калибровки скрапа. (Это обычно используется для сечений.) 
