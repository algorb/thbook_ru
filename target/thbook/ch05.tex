...
Точечные символы определяются как макросы с использованием команд\break |def ... enddef;|. Большинство определений точечных символов имеют четыре аргумента: положение (пара), вращение (числовое), масштаб (числовое) и выравнивание (пара). Исключения составляют {\it section (сечение)}, который не имеет визуального представления; все {\it labels (подписи)}, которые требуют специальной обработки, как описано в предыдущей главе; и {\it station\break (станция)}, которая принимает только один аргумент: позиция (пара). 
