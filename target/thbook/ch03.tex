...
* |format/fmt <формат>| = установить формат списка возможных продолжений (перспектив). В настоящее время поддерживаются следующие форматы вывода: |html| (по умолчанию), |txt|, \NEW{5.4}|kml|\[Для списка пещер и списка продолжений (перспектив).] и |dbf|. 
...
* |location <on/(off)>| = указать, следует ли экспортировать координаты входов пещеры в таблицу. * \NEW{5.3}|surveys (on)/off| = экспортирует необработанный список пещер при установке |off|. Иначе отображается структура съемки со статистикой. 
