...
* \NEW{5.4.1}|fonts-setup <tinysize> <smallsize> <normalsize> <largesize> <hugesize>| = указать размер текста в точечных символах. |<normalsize>| применяется к точечному символу `подпись', |<smallsize>| к `заметке' и всем другим точечным текстовым символам. Каждый из этих размеров может приме\-няться к строковому текстовому символу в соответствии с ее опцией |-размер|. 
