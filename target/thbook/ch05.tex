...
* arrow (стрелка) = числовая: 0 -- нет стрелок, 1 -- стрелка в конце, 2 -- в начале, 3 -- с обоих концов; * contour (контур) = текст: список точек, которые получают отметку или один из $-1$, $-2$ или $-3$ для отметки неопределенного метки, отметки в середине или без метки, соответственно; * section (сечение) = текст: список точек, которые получают стрелку ориентации или $-1$, чтобы указать стрелки; * slope (склон) = числовая: 0 -- нет границы, 1 -- есть граница; текст: список (точки, направления, длины) триплеты. \endlist
