...
\options * |namespace <on/off>| = указывает, создавать ли survey пространство имен (|on| по умолчанию) * |declination <specification>| = устанавливает склонение по умолчанию для всех объектов данных в этом survey (которые могут быть переопределены новым склонением в suburveys). The |<specification>| имеет три формы: 
