...
\NEW{5.3}{\bf Данные PocketTopo} экспортированные в формате Therion'а\[Это специальный текстовый формат, который нужно импортировать с помощью XTherion'а и не может обрабатываться непосредственно Therion'ом.] из приложения PocketTopo можно импортировать в текстовом редакторе, а также в редак\-торе карт ({\it Файл $\to$ Импорт $\to$ PocketTopo therion export} и {\it Фоновые изображения $\to$ Вставить $\to$ PocketTopo therion export}). Тот же файл используется для обоих импортов. Импорт эскиза напрямую не создает данные скрапа. Рисунок просто отображается на фоне как сканированные растровые изображения, и должен быть оцифрован вручную. 
