...
* GNU C/C++ компилятор. * GNU make. * Perl. * Python 2.7 или 3. * Tcl/Tk 8.4.3 или новее (\www{https://www.tcl.tk}) с набором виджетов {\it BWidget} \hfil\break (\www{https://sourceforge.net/projects/tcllib/}) и необязательным расширением\break {\it tkImg} \hfil\break (\www{https://sourceforge.net/projects/tkimg/}). 

%  {\it Tom} OpenGL extension (improved version is included in Therion source 
%  distribution)
%  and 
* \TeX\ дистрибутив с plain форматом \TeX, pdf\TeX, и \MP\ (\www{https://www.tug.org}). * LCDF Typetools пакет (\www{https://www.lcdf.org/type/}). * ImageMagick дистрибутив с утилитами {\it convert} и {\it identify}, если вы хотите использовать деформирование эскизы съемок. * {\it ghostscript} если вы хотите создать откалиброванные изображения карт с привязкой к географическим картам. \endlist
...
Все программы (за исключением пакетов BWidget и tkImg) обычно включаются в дистрибутивы Linux, Unix или MacOS X. Для Windows необходимы MinGW и MSYS (\www {http://www.mingw.org}). Эти дистрибутивы содержат GNU make и GCC. (Кстати, почему бы не использовать предварительно скомпилированную версию для Windows?). 
