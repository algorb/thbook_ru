...
|survey <id> [OPTIONS] ... other therion objects ... endsurvey [<id>]| \endsyntax
...
\options * |namespace <on/off>| = specify whether survey creates namespace (|on| by default) * |declination <specification>| = set the default declination for all data objects in this survey (which can be overridden by declination definitions in subsurveys). The |<specification>| имеет три формы: 
...
Если не определено склонение и определена какая-либо геодезическая система координат, склонение автоматически вычисляется с использованием встроенной геомагнитной модели. 

N.B.: склонение положительно, когда магнитный север находится к востоку от истинного севера. 
