...
* |projection <specification>| = указывает проекцию чертежа. Каждая проекция идентифицируется типом и дополнительно индексом в форме |тип[:индекс]|. Индекс может быть любым ключевым словом. Поддерживаются следующие типы проекций: 
