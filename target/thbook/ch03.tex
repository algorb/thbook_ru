...
|therion [-q] [-L] [-l <log-file>] [-s <source-file>] [-p <search-path>] [-b/--bezier] [-d] [-x] [--use-extern-libs] [<конфигурационный файл>]| 
...
|<конфигурационный файл>| Therion принимает только один необязатель\-ный аргумент: имя файла конфигурации. If no name is specified |thconfig| in the current directory is used. If there is no |thconfig| file (e.g.~current directory is not a data directory), Therion exits with an error message. \endarguments
