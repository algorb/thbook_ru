...
* |code <metapost/tex-map/tex-atlas>| = Добавить/переопределить макросы  \TeX\ и \MP\ . Это позволяет пользователю настраивать различные элементы (например, пользовательские символы, макет карты и атласа в одном месте \&c.) Для подробностей смотрите главу {\it Изменение макета}. * |endcode| = должен заканчивать разделы TeX\ и \MP\ sections \endcomopt
