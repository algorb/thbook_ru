...
Предположим ситуацию, когда вы хотите присоединиться к этим трем\break небольшим картам в рамках одного крупного проекта. Допустим, что координаты входных пещер указаны в файле верхнего уровня cave.3d. Если ваш код присоединения будет 
...
Это происходит потому, что в команде импорта верхнего уровня съемка {\it create} импортируется из файла |create/create.3d| с неправильными координатами, и мы импортируем файл .3d верхнего уровня с |-surveys use|. Это значит, что cave.create.1 будет импортироваться здесь как create.1@cave, а не 1@create.cave. 
