...
\leavevmode\kern1em\vbox{\advance\hsize-1em\halign{#\hfil\quad&#\hfil\cr {\it тип экспорта}&{\it поддерживаемые форматы}\cr\noalign{\smallskip\hrule\smallskip} model (модель) &loch, dxf, esri, compass, survex, vrml, 3dmf, kml\cr map (карта) &pdf, svg, xhtml, dxf, esri, survex, xvi, kml, \NEW{5.3}bbox\cr atlas &pdf\cr database &sql, \NEW{5.4}csv\cr lists &html, txt, kml, dbf\cr}}\egroup 
