...
Хотя кодировка Unicode UTF-8 содержит все символы, которые Therion может использовать, может быть неудобно использовать ее. В этом случае можно добавить поддержку любого 8-битной кодировки для текстовых входных файлов. Скопируйте файл перевода в |thchencdata| каталог; добавьте его имя в хэш-код `ifiles' в начале скрипта Perl |generate.pl|; запустить его и перекомпилируйте Therion. 

Файл перевода должен содержать два шестнадцатеричных символа (первый в 8-разрядной кодировке, второй в Unicode) в каждой строке. Можно добавлять комментарии за символом `|#|'. 

{\it Добавление новых \TeX\ кодировок} 

Легко добавить новые кодировки для вывода 2D-карты. \[\NEW{5.3}Этот раздел относится к выбору шрифтов старого стиля с использованием команды |tex-fonts| в файле инициализации и устарела при использовании команды |pdf-fonts|.] Скопируйте соответствующий файл сопоставления кодировки с |*.enc| расширение для в |texenc/encodings|, запустить скрипт Perl |mktexenc.pl| расположенный в каталоге |texenc| и перекомпилируйте Therion. 
...
{\it Генерация новых \TeX\ и \MP\ заголовков} 
...
* |THERION| = [не требуется] путь поиска для файла(ов) (x)therion.ini; * |HOME| (|HOMEDRIVE| + |HOMEPATH| на WinXP) = [не требуется, но обычно присутствует в вашей системе] путь поиска для файла(ов) (x)therion.ini; * |TEMP|, |TMP| = системный временный каталог, где Therion хранит временные файлы (в каталоге с именем |th$PID$|, где |$PID$| является идентификатором процесса), если только |tmp-path| указан в файле инициализации. \endlist
