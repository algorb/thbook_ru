...
* |location <on/(off)>| = указать, следует ли экспортировать координаты вхо\-дов пещеры в таблицу. * \NEW{5.3}|surveys (on)/off| = экспортирует необработан\-ный список пещер при установке |off|. Иначе отображается структура съемки со статистикой. 
