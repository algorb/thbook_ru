...
* \NEW{5.4.1}|min-symbol-scale <масштаб>| = определить минимальный |<масштаб>|, при кото\-ром точечные и линейные символы отображаются на карте. Например, при опции |min-symbol-scale M|, никакие точечные или линейные символы при масштабе |S|, и |XS| не будут показаны на карте. |<scale>| has the same format, as |scale| option for points and lines. 
